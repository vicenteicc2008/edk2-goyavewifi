...
static struct sdio_base_info sdio_ctrl_info[1] = {
{
	EMMC,
	SPRD_EMMC_BASE,
	REG_AP_CLK_EMMC_CFG,
...
static struct sdio_base_info *get_sdcontrol_info(uint32_t device_type)
{
...
static int mmc_startup(struct mmc *mmc)
{
	...
	debugf("mmc_set_capacity: part_num: %d", mmc->part_num);
	if (err) {
...
struct mmc *mmc_create(const struct mmc_config *cfg, void *priv)
{
	...
	mmc->block_dev.backstage_block_write = NULL;
	mmc->block_dev.backstage_write_query = NULL;
#ifdef CONFIG_EMMC_WP
				...
				unsigned long rpmbsize)
{
	...
	err = mmc_send_cmd(mmc, &cmd, NULL);
	if (err) {
		debugf("mmc_boot_partition_size_change: Error1 = %d\n", err);
		return err;
	}
	...
	err = mmc_send_cmd(mmc, &cmd, NULL);
	if (err) {
		debugf("mmc_boot_partition_size_change: Error2 = %d\n", err);
		return err;
	}
	...
	err = mmc_send_cmd(mmc, &cmd, NULL);
	if (err) {
		debugf("mmc_boot_partition_size_change: Error3 = %d\n", err);
		return err;
	}
	...
	err = mmc_send_cmd(mmc, &cmd, NULL);
	if (err) {
		debugf("mmc_boot_partition_size_change: Error4 = %d\n", err);
		return err;
	}
	return 0;
}
...
int mmc_boot_part_access(struct mmc *mmc, u8 ack, u8 part_num, u8 access)
{
	...
	if (err) {
		if (access)
			debugf("mmc boot partition#%d open fail:Error1 = %d\n",
			      part_num, err);
		else
			debugf("mmc boot partition#%d close fail:Error = %d\n",
			      part_num, err);
	...
	if (access) {
		...
		err = mmc_send_cmd(mmc, &cmd, NULL);
		if (err) {
			debugf("mmc boot partition#%d open fail:Error2 = %d\n",
			      part_num, err);
			return err;
		}
	}
